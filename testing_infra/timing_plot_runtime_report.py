...
def plot_csv_files(csv_files, labels):
    ...
    for csv_file, label, color in zip(csv_files, labels, palette):
        ...
        line, = plt.plot(df['matrix_dimension'], df['n_cycles'], marker='o', label=label, color=color)
        lines.append((label, df['n_cycles'], line))
    ...
    offset = 1.8  # Offset to move the text slightly above the lines
    for label, y_values, line in lines:
        plt.text(df['matrix_dimension'].iloc[-4], y_values.iloc[-4] + offset, label, fontsize=10, fontweight='bold', verticalalignment='bottom', color=line.get_color())
    ...
    plt.savefig("plot_runtime", dpi=300)
    plt.show()

...
def plot_csv_files(csv_files, labels, label_indices):
    ...
    for csv_file, label, index, color in zip(csv_files, labels, label_indices, palette):
        ...
        line, = plt.plot(df['matrix_dimension'], df['n_cycles'], marker='o',  markersize=3, label=label, color=color)
        lines.append((label, df['n_cycles'], line, index))
    ...
    offset = 20
    # Offset to move the text slightly above the lines
    ...
    plt.title('\nRuntime in s vs. input size', loc='left')
    ...
    plt.savefig("plot_runtime_trapeze_icc_report", dpi=300)

...
def plot_csv_files(csv_files, labels):
    ...
    for csv_file, label, color in zip(csv_files, labels, palette):
        ...
        line, = plt.plot(df['matrix_dimension'], df['n_performance'], marker='o', label=label, color=color)
        lines.append((label, df['n_performance'], line))
    
       # Adding labels directly to the lines within the graph with matching colors
    offset = 0.1  # Offset to move the text slightly above the lines
